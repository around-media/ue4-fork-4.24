...
bool FBAUtils::IsExecPin(const UEdGraphPin* Pin)
{
	...
	if (IsControlRigGraph(Graph))
	{
		...
		// 	{
		// 		return Struct->GetFName() == "ControlRigExecuteContext";
		// 	}
		// }
		//
		// return false;
...
bool FBAUtils::DoesNodeHaveExecutionTo(UEdGraphNode* NodeA, UEdGraphNode* NodeB)
{
	...
	while (!NodeQueue.IsEmpty())
	{
		...
		TArray<UEdGraphPin*> PinsToIterate;
		if (IsNodeImpure(NextNode))
		{
			PinsToIterate = GetLinkedToPins(NextNode).FilterByPredicate(IsExecPin);
		}
		else
		{
			PinsToIterate = GetLinkedToPins(NextNode);
		}

		for (UEdGraphPin* LinkedPin : PinsToIterate)
		{
...
void FBAUtils::PrintNodeInfo(UEdGraphNode* Node)
{
	...
	UE_LOG(LogBlueprintAssist, Warning, TEXT("### BEGIN print node <%s (%s)> <%s> <%s> <%d> info"),
			*EdGraphNode->GetName(),
			*EdGraphNode->GetClass()->GetFName().ToString(),
			*EdGraphNode->NodeGuid.ToString(),
			*EdGraphNode->GetGraph()->GetClass()->GetName(),
			EdGraphNode->GetUniqueID()
	);

	for (UEdGraphPin* Pin : EdGraphNode->Pins)
	{
		UE_LOG(LogBlueprintAssist, Warning, TEXT("\tPin <%s> | <%s> | <%s> | <%s> | <%s>"),
...
EBAGraphType FBAUtils::GetBAGraphType(UEdGraph* Graph)
{
	if (IsBehaviorTreeGraph(Graph))
	{
		return EBAGraphType::BehaviorTree;
	}

	if (IsSoundCueGraph(Graph))
	{
		return EBAGraphType::SoundCue;
	}

	if (IsMaterialGraph(Graph))
	{
		return EBAGraphType::MaterialGraph;
	}

	if (IsNiagaraGraph(Graph))
	{
		return EBAGraphType::NiagaraGraph;
	}

	if (IsAnimationGraph(Graph))
	{
		return EBAGraphType::AnimGraph;
	}

	if (IsBlueprintGraph(Graph))
	{
		return EBAGraphType::Blueprint;
	}

	if (IsControlRigGraph(Graph))
	{
		return EBAGraphType::ControlRigGraph;
	}

	if (IsMetasoundGraph(Graph))
	{
		return EBAGraphType::MetasoundGraph;
	}

	return EBAGraphType::Unknown;
}

FString FBAUtils::GetBAGraphTypeAsString(UEdGraph* Graph)
{
	switch (GetBAGraphType(Graph))
	{
		case EBAGraphType::Blueprint: return FString("Blueprint");
		case EBAGraphType::SoundCue: return FString("Sound Cue");
		case EBAGraphType::BehaviorTree: return FString("Behavior Tree");
		case EBAGraphType::MaterialGraph: return FString("Material Graph");
		case EBAGraphType::NiagaraGraph: return FString("Niagara Graph");
		case EBAGraphType::AnimGraph: return FString("Anim Graph");
		case EBAGraphType::ControlRigGraph: return FString("Control Rig Graph");
		case EBAGraphType::MetasoundGraph: return FString("Metasound Graph");
...
bool FBAUtils::IsAnimationGraph(UEdGraph* Graph)
{
	...
	return Graph->GetClass()->GetName() == "AnimationGraph";
}

bool FBAUtils::IsControlRigGraph(UEdGraph* Graph)
{
	if (Graph == nullptr)
	{
		return false;
	...
	// UE_LOG(LogBlueprintAssist, Warning, TEXT("Graph: %s"), *Graph->GetClass()->GetName());

	return Graph->GetClass()->GetName() == "ControlRigGraph";
	// return Cast<UEdGraphSchema_Niagara>(Graph->GetSchema()) != nullptr;
}

bool FBAUtils::IsMetasoundGraph(UEdGraph* Graph)
{
	if (Graph == nullptr)
	{
		return false;
	...
	return Graph->GetClass()->GetName() == "MetasoundEditorGraph";
}
...
bool FBAUtils::IsGraphNode(UEdGraphNode* Node)
{
	...
	return IsBlueprintNode(Node) || IsBehaviorTreeNode(Node) || IsMaterialNode(Node) || IsSoundCueNode(Node) || IsNiagaraNode(Node) || IsControlRigNode(Node) || IsMetasoundNode(Node);
...
{
	return IsK2Node(Node) && !IsCommentNode(Node) && !IsKnotNode(Node);
}
...
bool FBAUtils::IsControlRigNode(UEdGraphNode* Node)
{
	if (!Node)
	{
		return false;
	...
	{
		FString NodeType = Node->DEPRECATED_NodeWidget.Pin()->GetTypeAsString();
		return NodeType.Contains("ControlRig");
	}

	return IsControlRigGraph(Node->GetGraph());
}

bool FBAUtils::IsMetasoundNode(UEdGraphNode* Node)
{
	if (!Node)
	{
		return false;
	...
	if (Node->DEPRECATED_NodeWidget.IsValid())
	{
		const FString NodeType = Node->DEPRECATED_NodeWidget.Pin()->GetTypeAsString();
		return NodeType.Contains("Metasound");
	}
...
TSharedPtr<WidgetClass> FBAUtils::GetChildWidgetCasted(TSharedPtr<SWidget> Widget, const FString& WidgetClassName, bool bCheckContains)
{
...
{
	return (GraphPanel->GetTickSpaceGeometry().LocalToAbsolute(PanelSpaceCoordinate) - GraphPanel->
...
FBAFormatterSettings& FBAUtils::GetFormatterSettings(UEdGraph* Graph)
{
	...
	switch (GraphType)
	{
		...
		case EBAGraphType::ControlRigGraph:
			return BASettings->ControlRigGraphFormatterSettings;
		case EBAGraphType::MetasoundGraph:
			return BASettings->MetasoundGraphFormatterSettings;
		default:
...
AActor* FBAUtils::GetSCSNodeDefaultActor(TSharedPtr<FSCSEditorTreeNode> Node, UBlueprint* Blueprint)
{
...
bool FBAUtils::IsSCSActorNode(TSharedPtr<FSCSEditorTreeNode> Node)
{
#if ENGINE_MINOR_VERSION >= 26 || ENGINE_MAJOR_VERSION >= 5
...
bool FBAUtils::HasMetaDataChanged(const FBPVariableDescription& OldVariable, const FBPVariableDescription& NewVariable, const FName Key)
{
	...
	{
		return !OldVariable.GetMetaData(Key).Equals(NewVariable.GetMetaData(Key));
	}
...
{
	return FBAAssetEditorHandler::Get().GetEditorFromTab(FBATabHandler::Get().GetLastMajorTab());
}
...
EditorClass* FBAUtils::GetEditorFromActiveTabCasted()
{
	return FBAAssetEditorHandler::Get().GetEditorFromTabCasted<AssetClass, EditorClass>(FBATabHandler::Get().GetLastMajorTab());
}


template <class AssetClass>
AssetClass* FBAUtils::GetAssetForActiveTab()
{
	return FBAAssetEditorHandler::Get().GetAssetFromTab<AssetClass>(FBATabHandler::Get().GetLastMajorTab());
}
