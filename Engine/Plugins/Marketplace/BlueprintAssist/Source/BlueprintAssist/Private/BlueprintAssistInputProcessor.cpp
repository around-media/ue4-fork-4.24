...
{
	if (GEditor->bIsSimulatingInEditor || GEditor->PlayWorld != nullptr || FBAUtils::IsCompilingCode())
	{
		return;
	}
	...
	FBAAssetEditorHandler::Get().Tick();
}
...
bool FBAInputProcessor::HandleKeyDownEvent(FSlateApplication& SlateApp, const FKeyEvent& InKeyEvent)
{
	...
	if (SlateApp.IsInitialized())
	{
		...
		// cancel processing node sizes when you press esc
...
void FBAInputProcessor::LinkNodesBetweenWires()
{
	...
	if (FBAUtils::GetFormatterSettings(Graph).AutoFormatting == EBAAutoFormatting::FormatSingleConnected)
	{
	...
	for (const FPinLink& Link : PendingLinks)
	{
		...
		if (bMadeLink)
		{
			if (FBAUtils::GetFormatterSettings(Graph).AutoFormatting != EBAAutoFormatting::Never)
			{
...
void FBAInputProcessor::DuplicateNodeForEachLink() const
{
	...
	if (!FBAUtils::IsNodePure(SelectedNode))
	{
		FNotificationInfo Notification(
			FText::FromString(
				"Duplicate Node For Each Link currently only supports pure nodes"));
	...
	for (UEdGraphPin* LinkedToPin : LinkedPins)
	{
		...
		for (TSet<UEdGraphNode*>::TIterator It(PastedNodes); It; ++It)
		{
			...
			Node->NodePosX = FBAUtils::GetPinPos(GraphHandler, LinkedToPin).X;
	...
	auto AutoFormatting = FBAUtils::GetFormatterSettings(DestinationGraph).AutoFormatting;

	if (AutoFormatting != EBAAutoFormatting::Never)
	{
		for (UEdGraphPin* LinkedToPin : LinkedPins)
		{
			GraphHandler->AddPendingFormatNodes(LinkedToPin->GetOwningNode(), Transaction);
		}
	}
}
