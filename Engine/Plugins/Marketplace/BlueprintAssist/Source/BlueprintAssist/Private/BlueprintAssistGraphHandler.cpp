...
void FBAGraphHandler::InitGraphHandler()
{
	...
	SaveNodesSizes(GetFocusedEdGraph()->Nodes);
}
...
bool FBAGraphHandler::TryAutoFormatNode(UEdGraphNode* NewNodeToFormat, TSharedPtr<FScopedTransaction> InPendingTransaction)
{
...
void FBAGraphHandler::SaveNodesSizes(const TArray<UEdGraphNode*>& Nodes)
{
	// check all nodes on the graph
	for (UEdGraphNode* Node : Nodes)
	{
		...
		{
			if (!GetDefault<UBASettings>()->bCacheSizeWhenRequired)
			{
...
void FBAGraphHandler::AutoAddParentNode(UEdGraphNode* NewNode)
{
	...
	{
		auto Schema = Cast<UEdGraphSchema_K2>(GetFocusedEdGraph()->Schema);
		...
		if (ValidParent && TargetGraph)
		{
			...
			if (GetDefault<UBASettings>()->BlueprintFormatterSettings.AutoFormatting != EBAAutoFormatting::Never)
			{
				AddPendingFormatNodes(NewNode);
				AddPendingFormatNodes(ParentFunctionNode);
			}
		}
	}
}
...
void FBAGraphHandler::SetDefaultSizeForFocusedNode()
{
	// set default size to 300, 150
	PendingSize.RemoveSwap(FocusedNode);

	const TSharedPtr<SGraphNode> GraphNode = GetGraphNode(FocusedNode);
	...
	NodeData.CachedNodeSize = NodeSize;
	GetGraphCache().CachedNodes.Add(FocusedNode->NodeGuid, NodeData);
}
...
UEdGraphNode* FBAGraphHandler::GetRootNode(UEdGraphNode* InitialNode, const TArray<UEdGraphNode*>& NodesToFormat, bool bCheckSelectedNode)
{
	...
	for (UEdGraphNode* Node : NodeTree)
	{
		...
		if (FBAUtils::IsExtraRootNode(Node) && FBAUtils::DoesNodeHaveExecutionTo(Node, InitialNode))
		{
			RootNodes.Add(Node);
			continue;
		}
		...
		{
			if (FBAUtils::IsEventNode(Node, FormatterDirection) && FBAUtils::DoesNodeHaveExecutionTo(Node, InitialNode))
			{
				EventNodes.Add(Node);
				continue;
			}
			...
			if (LinkedInputPins.Num() == 0 && FBAUtils::DoesNodeHaveExecutionTo(Node, InitialNode))
			{
...
void FBAGraphHandler::AddPendingFormatNodes(UEdGraphNode* Node, TSharedPtr<FScopedTransaction> InPendingTransaction, FEdGraphFormatterParameters InFormatterParameters)
{
	...
	if (GetMutableDefault<UBASettings>()->bCacheSizeWhenRequired)
	{
		...
		{
			if (FBAUtils::IsGraphNode(ConnectedNode) &&
				!FBAUtils::IsKnotNode(ConnectedNode) &&
				!GetGraphCache().CachedNodes.Contains(ConnectedNode->NodeGuid) &&
				!PendingSize.Contains(ConnectedNode))
			{
...
void FBAGraphHandler::UpdateNodesRequiringFormatting()
{
	...
	if (PendingFormatting.Num() == 0)
	{
		FormatterParameters.Reset();
		PendingTransaction.Reset();
		// UE_LOG(LogBlueprintAssist, Warning, TEXT("Reset transactions?"));
	}
...
void FBAGraphHandler::CancelProcessingNodeSizes()
{
	...
	{
		FBANodeData NodeData;
		NodeData.CachedNodeSize = FVector2D(300, 150);
		GetGraphCache().CachedNodes.Add(Node->NodeGuid, NodeData);
	}
