...
struct BLUEPRINTASSIST_API FBAUtils
{
	...
	static EBAGraphType GetBAGraphType(UEdGraph* Graph);

	static FString GetBAGraphTypeAsString(UEdGraph* Graph);

	static FFormatterInterface* MakeFormatter(EBAGraphType GraphType);
	...
	static bool IsMetasoundGraph(UEdGraph* Graph);
	...
	static bool IsControlRigNode(UEdGraphNode* Node);

	static bool IsMetasoundNode(UEdGraphNode* Node);
	...
	static AActor* GetSCSNodeDefaultActor(TSharedPtr<FSCSEditorTreeNode> Node, UBlueprint* Blueprint);

	static bool IsSCSActorNode(TSharedPtr<FSCSEditorTreeNode> Node);
